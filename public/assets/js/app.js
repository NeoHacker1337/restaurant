...
function handleDropdownMenu() {
    const dropdownToggleButtons = document.querySelectorAll('.dropdown-button'); // Updated selector
    dropdownToggleButtons.forEach((button) => {
        const content = button.nextElementSibling;
        if (!content) return; // Skip if no content

        button.addEventListener('click', () => {
            console.log('Dropdown clicked:', button);
...
function updateActiveBtn(btnId) {
    console.log("updateActiveBtn called with btnId:", btnId);
...
function layoutSwitch(isLayoutAttributes) {
    try {
        ...
        console.error("Error in layoutSwitch:", error);
    }
}
