...
<body
    ...
    if ($user->role == 'admin') {
      ...
      $profile = route('admin-profile.index');
    }

    if ($user->role == 'customer') {
        //return '';
        $dashboard = route('customer.dashboard');
        $profile = route('customer-profile.index');
    }

    if ($user->role == 'vendor') {
        //return '';
        $dashboard = route('vendor.dashboard');
        $profile = route('restaurant-profile.index');
    }

    if ($user->role == 'delivery') {
        //return '';
        $dashboard = route('delivery.dashboard');
        $profile = route('delivery-profile.index');
    }
   @endphp
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Select all dropdown buttons and content elements
            ...
            document.addEventListener('click', function(event) {
                dropdownButtons.forEach((dropdownButton, index) => {
                    const dropdownContent = dropdownContents[index];
                    if (!dropdownButton.contains(event.target) && !dropdownContent.contains(event
                            .target)) {
                        dropdownContent.classList.add(
                            'hidden'); // Hide the dropdown if clicked outside
                    }
                });
            });
        });
